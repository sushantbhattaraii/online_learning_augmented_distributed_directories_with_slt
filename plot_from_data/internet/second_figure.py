...
for cutoff in cutoffs:
    # Construct filenames for 256 and 512 nodes
    file_512 = f"512nodes_diameter7_cutoff{cutoff}-repetitions50-overlap100.xlsx"
    
    # Load and aggregate data for 512 nodes
    df_512 = pd.read_excel(f"./../../results_spt_implementation/internet_graphs/second/{file_512}")
...
plt.plot([str(x) for x in categories], results['mean_stretch_SPT_512'], marker='s', linestyle='--', label=f'SPT', color=cmap(6), linewidth=1.1, markersize=4, zorder=4)
...
# Save the plot
# plt.savefig('second_internet.png')
plt.show()
