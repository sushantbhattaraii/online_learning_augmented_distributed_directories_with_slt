...
for n in node_sizes:
    df = pd.read_excel(f"./../../results_spt_implementation/internet_graphs/first/{file_mapping[n]}")
    ...
    stats['stretch_SPT_f128'].append(df[df['fraction'] == 128]['stretch_SPT'].mean())
...
plt.plot([str(x) for x in node_sizes], stats['stretch_SPT_f128'], marker='s', linestyle='--', label=f'SPT', color=cmap(6), linewidth=1.1, markersize=4, zorder=4)
